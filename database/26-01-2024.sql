...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 26-01-2024 a las 20:01:39
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_filtrar_fecha_reportes_administrador` (IN `_inicial` DATETIME, IN `_final` DATETIME)   SELECT reporte.id_reporte, reporte.fecha_hora,reporte.turno,CONCAT(usuarios.nombres,' ',usuarios.apellidos) AS jefe_fundo,reporte.nom_fundo,lote.nombre_lote,sub_lote._slote_nombre, cultivo.nombre_cultivo, variedad.nombre_variedad,reporte.aprob_jefefundo,reporte.aprob_jefesanidad, motivo_aplicacion.nombre_motivo,productos_sanidad.nombre_producto,detalle_reporte.diascarencia, detalle_reporte.dosiscil, detalle_reporte.ncil,detalle_reporte.dosistanque,detalle_reporte.totalproducto, detalle_reporte.dosisHA,detalle_reporte.HAaplicada,detalle_reporte.gastoH2O FROM reporte
INNER JOIN usuarios ON reporte.fk_jefe_fundo= usuarios.idusuario
INNER JOIN lote ON reporte.fk_lote=lote.id_lote
INNER JOIN sub_lote ON reporte.fk_slote= sub_lote.id_sub_lote
INNER JOIN cultivo ON reporte.fk_cultivo=cultivo.id_cultivo
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_listar_datos_reporte` (IN `_idReporte` INT)   SELECT CONCAT(usuarios.nombres,' ',usuarios.apellidos) AS jefe_fundo , reporte.nom_fundo,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_listar_reportes_porJefeFundo` (IN `_jefe` INT)   SELECT reporte.id_reporte, reporte.fecha_hora,reporte.turno,CONCAT(usuarios.nombres,' ',usuarios.apellidos) AS jefe_fundo,reporte.nom_fundo,lote.nombre_lote,sub_lote._slote_nombre, cultivo.nombre_cultivo, variedad.nombre_variedad,reporte.aprob_jefefundo,reporte.aprob_jefesanidad FROM reporte
INNER JOIN usuarios ON reporte.fk_jefe_fundo= usuarios.idusuario
INNER JOIN lote ON reporte.fk_lote=lote.id_lote
INNER JOIN sub_lote ON reporte.fk_slote= sub_lote.id_sub_lote
INNER JOIN cultivo ON reporte.fk_cultivo=cultivo.id_cultivo
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_reporte_administrador_listar` ()   SELECT reporte.id_reporte, reporte.fecha_hora,reporte.turno,CONCAT(usuarios.nombres,' ',usuarios.apellidos) AS jefe_fundo,reporte.nom_fundo,lote.nombre_lote,sub_lote._slote_nombre, cultivo.nombre_cultivo, variedad.nombre_variedad,reporte.aprob_jefefundo,reporte.aprob_jefesanidad, motivo_aplicacion.nombre_motivo,productos_sanidad.nombre_producto,detalle_reporte.diascarencia, detalle_reporte.dosiscil, detalle_reporte.ncil,detalle_reporte.dosistanque,detalle_reporte.totalproducto, detalle_reporte.dosisHA,detalle_reporte.HAaplicada,detalle_reporte.gastoH2O FROM reporte
INNER JOIN usuarios ON reporte.fk_jefe_fundo= usuarios.idusuario
INNER JOIN lote ON reporte.fk_lote=lote.id_lote
INNER JOIN sub_lote ON reporte.fk_slote= sub_lote.id_sub_lote
INNER JOIN cultivo ON reporte.fk_cultivo=cultivo.id_cultivo
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_reporte_pdf` (IN `_idreporte` INT)   SELECT reporte.id_reporte, reporte.fecha_hora,reporte.turno,CONCAT(usuarios.nombres,' ',usuarios.apellidos) AS jefe_fundo,usuarios.sello,reporte.nom_fundo,reporte.enc_sanidad, reporte.enc_QA,reporte.enc_almacen, lote.nombre_lote,sub_lote._slote_nombre, cultivo.nombre_cultivo, variedad.nombre_variedad,reporte.aprob_jefefundo,reporte.aprob_jefesanidad FROM reporte INNER JOIN usuarios ON reporte.fk_jefe_fundo= usuarios.idusuario INNER JOIN lote ON reporte.fk_lote=lote.id_lote INNER JOIN sub_lote ON reporte.fk_slote= sub_lote.id_sub_lote INNER JOIN cultivo ON reporte.fk_cultivo=cultivo.id_cultivo INNER JOIN variedad ON reporte.fk_variedad= variedad.id_variedad WHERE reporte.id_reporte=_idreporte$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_reporte_registro` (IN `_fecha_hora` DATETIME, IN `_turno` VARCHAR(15), IN `_enc_sanidad` VARCHAR(50), IN `_enc_QA` VARCHAR(50), IN `_enc_almacen` VARCHAR(50), IN `_fundo` INT, IN `_nonfundo` VARCHAR(30), IN `_lote` INT, IN `_slote` INT, IN `_cultivo` INT, IN `_variedad` INT)   INSERT INTO `reporte`( `fecha_hora`, `turno`, `enc_sanidad`, `enc_QA`, `enc_almacen`, `fk_jefe_fundo`, `nom_fundo`, `fk_lote`, `fk_slote`, `fk_cultivo`, `fk_variedad`, `aprob_jefefundo`, `aprob_jefesanidad`) VALUES (NOW(),_turno,_enc_sanidad,_enc_QA,_enc_almacen,_fundo,_nonfundo,_lote,_slote,_cultivo,_variedad,'No Aprobado','No Aprobado')$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_usuarios_modificar` (IN `_idusuario` INT, IN `_nombreusuario` VARCHAR(25), IN `_nivelacceso` CHAR(1), IN `_sello` TEXT, IN `_email` VARCHAR(50))   BEGIN
	UPDATE usuarios SET
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_usuarios_registro` (IN `_nombres` VARCHAR(50), IN `_apellidos` VARCHAR(50), IN `_nombreusuario` VARCHAR(25), IN `_nivelacceso` CHAR(1), IN `_sello` TEXT, IN `_email` VARCHAR(80))   BEGIN
	INSERT INTO usuarios(nombres, apellidos,nombreusuario, clave, fechacreacion,fechabaja, nivelacceso, estado,sello,  email, codverificacion)VALUES
			(_nombres, _apellidos, _nombreusuario, "$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi", CURDATE(), NULL, _nivelacceso, "A", _sello,_email, NULL);
END$$
...
(23, 10, 10, 'L', 4, '5', '1234', '4', '8', '2', '8', '1200', 23),
(24, 10, 27, 'L', 4, '2', '1234', '5', '10', '2', '10', '1200', 23),
(25, 15, 24, 'KG', 2, '2', '4453', '4', '8', '5', '2', '1500', 24),
(26, 16, 25, 'L', 3, '3', '4523', '3', '8', '3', '2', '1500', 24),
(27, 10, 18, 'L', 3, '4523', '345', '2', '4', '3', '2', '1500', 24),
(28, 9, 26, 'KG', 2, '2', '2', '2', '2', '2', '2', '2', 25),
(29, 10, 23, 'KG', 2, '2', '2', '2', '1', '1', '1', '1', 25),
(30, 8, 22, 'L', 1, '1', '1', '1', '1', '1', '1', '1', 24),
(31, 12, 13, 'KG', 1, '1', '1', '1', '1', '1', '1', '1', 24),
(32, 9, 14, 'KG', 1, '1', '1', '1', '1', '1', '1', '1', 24),
(33, 10, 18, 'L', 1, '1', '1', '1', '1', '1', '1', '1', 24),
(34, 13, 23, 'KG', 1, '1', '1', '1', '1', '1', '1', '1', 24),
(35, 20, 18, 'L', 1, '1', '1', '1', '1', '1', '1', '1', 24),
(36, 10, 18, 'L', 1, '1', '1', '1', '1', '1', '1', '1', 24),
(37, 20, 18, 'L', 1, '1', '1', '1', '1', '1', '1', '1', 24),
(38, 20, 18, 'L', 1, '1', '1', '1', '1', '1', '1', '1', 24);
...
INSERT INTO `reporte` (`id_reporte`, `fecha_hora`, `turno`, `enc_sanidad`, `enc_QA`, `enc_almacen`, `fk_jefe_fundo`, `fk_jefeSanidad`, `nom_fundo`, `fk_lote`, `fk_slote`, `fk_cultivo`, `fk_variedad`, `aprob_jefefundo`, `aprob_jefesanidad`) VALUES
(23, '2024-01-25 06:34:07', 'Mañana', 'Pablo Peralta', 'Marcos Rojas', 'Victoria Sifuentes', 16, 31, 'Santa Margarita', 3, 59, 1, 1, 'Aprobado', 'Aprobado'),
(24, '2024-01-26 06:36:46', 'Mañana', 'Diego Altamirano', 'Junior Espilco', 'Diana Yauri', 28, 31, 'SANTA INES', 44, 104, 6, 20, 'Aprobado', 'Aprobado'),
(25, '2024-01-26 09:31:14', 'Mañana', 'Miguel torres', 'Miguel torres', 'Miguel torres', 29, NULL, 'ALPINE PERU', 56, 117, 10, 25, 'Aprobado', 'No Aprobado'),
(26, '2024-01-26 13:13:22', 'Tarde', 'a', 'a', 'a', 26, NULL, 'DOS MARIAS', 41, 100, 6, 19, 'No Aprobado', 'No Aprobado'),
(27, '2024-01-26 13:15:52', 'Tarde', 'b', 'b', 'b', 25, NULL, 'DOS MARIAS', 40, 99, 6, 19, 'No Aprobado', 'No Aprobado'),
(28, '2024-01-26 13:16:56', 'Tarde', 'c', 'c', 'c', 29, NULL, 'ALPINE PERU', 55, 116, 10, 24, 'No Aprobado', 'No Aprobado'),
(29, '2024-01-26 13:29:21', 'Tarde', 'v', 'v', 'v', 24, NULL, 'SANTA LUCIA', 37, 94, 4, 18, 'No Aprobado', 'No Aprobado');
...
INSERT INTO `usuarios` (`idusuario`, `nombres`, `apellidos`, `nombreusuario`, `clave`, `fechacreacion`, `fechabaja`, `nivelacceso`, `estado`, `sello`, `email`, `codverificacion`, `ingreso`) VALUES
(13, 'Administrador', 'Administrador', 'Administrador', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-10', NULL, 'A', 'A', '', 'admin@admin.com', NULL, 43),
(16, 'Amadeo', 'Acuña', 'aacuña', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-16', NULL, 'J', 'A', '20240126021435.jpg', 'uva1s@beta.com', NULL, 11),
(17, 'Roberto', 'Bernales', 'rbernales', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-16', NULL, 'J', 'A', '20240125071905.jpg', 'uva1s@beta.com', NULL, 1),
(18, 'Roque', 'Sifuentes', 'rsifuentes', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-25', NULL, 'J', 'A', '20240125071919.jpg', 'uva1s@beta.com', NULL, 0),
...
(20, 'Gary', 'Trujillo', 'gtrujillo', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-16', NULL, 'J', 'A', '', 'uva1s@beta.com', NULL, 3),
...
(26, 'Eder', 'Razabal', 'erazabal', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-16', NULL, 'J', 'A', '20240125071954.jpg', 'uva1s@beta.com', NULL, 0),
...
(28, 'Miguel', 'Vallardares', 'mvallardares', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-16', NULL, 'J', 'A', '20240126041603.jpg', 'uva1s@beta.com', NULL, 2),
(29, 'Gino', 'Rosas', 'grosas', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-16', NULL, 'J', 'A', '', 'uva1s@beta.com', NULL, 1),
(31, 'Sanidad', 'Sanidad', 'Sanidad', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-17', NULL, 'S', 'A', '20240126041642.jpg', 'uva1s@beta.com', NULL, 7),
(32, 'Operario', 'Operario', 'Operario', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-17', NULL, 'O', 'A', '', 'uva1s@beta.com', NULL, 20),
(33, 'Calidad', 'Calidad', 'Calidad', '$2y$10$dvgzm2Jmh0u98DerZSGkX.QH5rVqqD/ctSC3UCgYNj4jFh0CgR5mi', '2024-01-17', NULL, 'C', 'A', '', 'uva1s@beta.com', NULL, 5);
...
--
ALTER TABLE `detalle_reporte`
  MODIFY `id_detallereporte` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=39;
