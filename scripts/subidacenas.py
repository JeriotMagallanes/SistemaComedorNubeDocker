...
def process_students():
    ...
    try:
        ...
        query = "SELECT idAsistencia, fecha_hora, dni FROM asistenciacomedorcena WHERE server = 0"
        ...
        for row in result:
            ...
            url = f"http://182.160.24.51:8080/controllers/asistenciaCena.controller.php?op=registrarAsistenciaCena&fecha_hora={fecha_hora}&dni={dni}"
            try:
                response = requests.get(url)
                if response.status_code == 200:
                    # Actualizar el atributo 'server' a 1
                    update_query = "UPDATE asistenciacomedorcena SET server = 1 WHERE idAsistencia = %s"
