...
def process_students():
    ...
    try:
        ...
        query = "SELECT idAsistencia, fecha_hora, dni FROM asistenciacomedoralmuerzo WHERE server = 0"
        ...
        for row in result:
            ...
            url = f"http://182.160.24.51:8080/controllers/asistenciaAlmuerzo.controller.php?op=registrarAsistenciaalmuerzo&fecha_hora={fecha_hora}&dni={dni}"
            try:
                response = requests.get(url)
                if response.status_code == 200:
                    # Actualizar el atributo 'server' a 1
                    update_query = "UPDATE asistenciacomedoralmuerzo SET server = 1 WHERE idAsistencia = %s"
